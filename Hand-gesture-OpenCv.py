...
while True:
    ...
    if num_frames < 60:
        ...
        if hand is not None:
            ...
            # Display "None" if there is no hand geture
            if len(res)==0:
                cv2.putText(frame_copy, str("Gesture: None"), (70, 45), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
            else:
                # From predict function, classify the hand gesture using index number
                match str(res[0]):
                    case "0":
                        x = "Fist"
                    case "1":
                ...
                # Display the hand gesture text on the video window
                cv2.putText(frame_copy, str("Gesture: "+x), (70, 45), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
